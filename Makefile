...
LIBMOSQUITTO := -L$(MOSQUITTO)/lib $(MOSQUITTO)/lib/cpp/libmosquittopp.so.1 $(MOSQUITTO)/lib/libmosquitto.so.1
...
INCLUDES := -I $(OPENZWAVE)/cpp/src -I $(OPENZWAVE)/cpp/src/command_classes/ \
	...
	-I /usr/local/include/ -I $(MOSQUITTO)/lib
...
LIBUSB := -ludev
LIBS := $(LIBZWAVE) $(GNUTLS) $(LIBMICROHTTPD) $(LIBMOSQUITTO) -pthread $(LIBUSB) -lresolv
